...
function ProductSaleItem(props) {
  ...
  const handleAddToCart = () => {
    if(user){
      ...
      const productData = {
        ...
        color: selectedColor ? props.product.product.colors.find(color => color.id == selectedColor).color : '',
        qty: selectedQuantity, // Assuming quantity is always 1 when adding to cart
        size: selectedSize ? props.product.product.find(size => size.id == selectedSize).size : '',
        user: {
          id: user.userId
        }
      };
  ...
  //   console.log("Adding to wishlist:", props.product.productId.id,props.product.image); 
  //   WishlistService.addToWishlist(props.product.productId.id,props.product.image)
  //     .then(() => {
  ...
  //     })
  //     .catch((error) => {
  //   onClick={handleAddToWishlist}    console.error("Error adding to wishlist: ", error);
  //     });
  // };
    return ( 
      ...
      <div className="product product-11 text-center">
        ...
        <span className="product-label label-circle label-sale">
                      ...
                      <Link to={`/productdetail/${props.product.product.id}`}>
  <img
  ...
  />
        ...
        <div className="product-body">
          <div className="product-cat">
            <a href="#">{props.product.product.brand.name}</a>
          </div>
          {/* End .product-cat */}
          <h3 className="product-title">
            <a href="product.html">{props.product.product.name}</a>
          </h3>
          {/* End .product-title */}
          <div style={{ textDecoration: 'line-through', color: '#999' }} className="product-price-sale">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(props.product.product.price)}</div>
    <div className="product-price"><h6> {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(props.product.salePrice)}</h6></div>
          {/* End .product-price */}
        </div>
        {/* End .product-body */}
        ...
        <div className="modal" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 0, 0, 0.5)', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }}>
          <div className="modal-content" style={{ width: '400px', height: '400px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }}>
            ...
            <select value={selectedSize} onChange={(e) => setSelectedSize(e.target.value)} style={{ marginBottom: '10px', width: '100%', padding: '10px', border: '1px solid #ccc', borderRadius: '5px', fontSize: '16px' }}>
              {props.product.product.sizes.map((size, index) => (
            ...
            <div style={{ marginBottom: '10px', width: '100%', display: 'flex', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                {props.product.product.colors.map((color, index) => (
